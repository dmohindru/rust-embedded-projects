...
type StartPauseType = Mutex<ThreadModeRawMutex, bool>;

static FRAME_CURSOR: FrameCursorType = Mutex::new(None);
static STEP_CURSOR: StepCursorType = Mutex::new(None);
static START_PAUSE: StartPauseType = Mutex::new(false);
...
//--------------------
#[embassy_executor::task]
async fn pause_button_task(mut button: DebouncedButton<Input<'static>, embassy_time::Delay>) {
    button
        .wait(|| async {
            {
                let mut pause: MutexGuard<'_, ThreadModeRawMutex, bool> = START_PAUSE.lock().await;
                *pause = !*pause;
            }
        })
        .await;
...
async fn timer_task() {
    {
        loop {
            ...
            // ---- 3. Read pause button task (copy, cheap) ----
            let is_pause = {
                let start_pause_guard: MutexGuard<'_, ThreadModeRawMutex, bool> =
                    START_PAUSE.lock().await;
                *start_pause_guard
            }; // start/pause guard dropped here
            ...
            // ---- 4. Move frame cursor ----
            {
                ...
                if let Some(frame_cursor) = frame_cursor_guard.as_mut().filter(|_| !is_pause) {
